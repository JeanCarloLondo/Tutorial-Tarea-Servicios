...
public class ProductServiceImpl implements ProductService {
    ...
    public Page<Product> getAllProducts(Pageable pageable) {
        logger.debug("Obteniendo productos activos con paginación: {}",
                pageable);
    ...
    public Product createProduct(Product product) {
        ...
        logger.info("Producto creado exitosamente con ID: {}",
                savedProduct.getId());
        return savedProduct;
    }
    ...
    public List<Product> getProductsByPriceRange(BigDecimal minPrice,
            BigDecimal maxPrice) {
        logger.debug("Buscando productos en rango de precio: {} - {}",
                minPrice, maxPrice);
    ...
    public Product updateStock(Long id, Integer newStock) {
        logger.debug("Actualizando stock del producto {}: nuevo stock = {}",
                id, newStock);
        Product product = productRepository.findByIdAndActiveTrue(id)
                .orElseThrow(() -> new ProductNotFoundException("Producto no encontrado con ID: " + id));
        if (newStock < 0) {
