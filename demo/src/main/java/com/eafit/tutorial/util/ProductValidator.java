 ...
 *
 * Implementa validaciones complejas que van más allá de las anotaciones
 * básicas.
 */
@Component
public class ProductValidator {
    private static final String[] FORBIDDEN_WORDS = { "test", "prueba",
            "demo", "temporal" };
    ...
    public void validateForCreation(CreateProductDTO productDTO) {
        ...
        if (containsForbiddenWords(productDTO.getName())) {
            errors.put("name",
                    "El nombre del producto no puede contener palabras prohibidas como 'test', 'demo', etc.");
        }
        ...
        validatePriceCategoryCoherence(productDTO.getPrice(),
                productDTO.getCategory(), errors);
        if (!errors.isEmpty()) {
            throw new ValidationException("Error de validación de reglas denegocio", errors);
        }
    }
    ...
    private void validatePriceCategoryCoherence(BigDecimal price, String category, Map<String, String> errors) {
        ...
        if (lowerCategory.contains("ropa") ||
                lowerCategory.contains("vestimenta")) {
    ...
    public void validateStockForCategory(String category, Integer stock) {
        ...
        if ((lowerCategory.contains("digital") ||
                lowerCategory.contains("software")) && stock < 1000) {
        ...
        if ((lowerCategory.contains("comida") ||
                lowerCategory.contains("alimento")) && stock > 100) {
